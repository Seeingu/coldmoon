...
func ContinueDynamicImport(agent *Agent, capability *PromiseCapability, moduleCompletion CompletionModule) {
	if moduleCompletion.IsAbrupt() {
		NewValueFromObject(capability.Reject).CallAssumeCallable(UndefinedValue, []Value{moduleCompletion.Error()})
		return
	}
	...
	var rejectedClosure BehaviorFn = func(thisArgument Value, argumentsList []Value, newTarget ObjectType) Value {
		NewValueFromObject(capability.Reject).CallAssumeCallable(UndefinedValue, []Value{argumentsList[0]})
		return nil
	}
	...
	var linkAndEvaluateClosure BehaviorFn = func(thisArgument Value, argumentsList []Value, newTarget ObjectType) Value {
		link := module.Link()
		if link.IsAbrupt() {
			NewValueFromObject(capability.Reject).CallAssumeCallable(UndefinedValue, []Value{link.Error()})
			return nil
		}
		...
		var fulfilledClosure BehaviorFn = func(thisArgument Value, argumentsList []Value, newTarget ObjectType) Value {
			...
			NewValueFromObject(capability.Resolve).CallAssumeCallable(UndefinedValue, []Value{NewValueFromObject(namespace)})
			return nil
		}
		onFulfilled := CreateBuiltinFunction(agent, fulfilledClosure, 0, "", builtinFunctionArgs{})
		PerformPromiseThen(agent, evaluatePromise, NewValueFromObject(onFulfilled), NewValueFromObject(onRejected), nil)
		return nil
	}

	linkAndEvaluate := CreateBuiltinFunction(agent, linkAndEvaluateClosure, 0, "", builtinFunctionArgs{})
	PerformPromiseThen(agent, loadPromise, NewValueFromObject(linkAndEvaluate), NewValueFromObject(onRejected), nil)
}
